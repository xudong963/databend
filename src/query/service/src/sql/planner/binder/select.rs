...
impl<'a> Binder {
    ...
    fn bind_union(
        ...
        dbg!(coercion_types.clone());
        let (new_bind_context, left_expr, right_expr) = self.coercion_union_type(
            left_context,
            right_context,
            left_expr,
            right_expr,
    ...
    fn coercion_union_type(
        ...
        let mut left_project_column_set = Vec::with_capacity(left_bind_context.columns.len());
        let mut right_project_column_set = Vec::with_capacity(right_bind_context.columns.len());
            ...
            .enumerate()
        {
            if left_col.data_type != coercion_types[idx] {
                ...
                left_project_column_set.push(new_column_index);
                new_bind_context.add_column_binding(column_binding);
            } else {
                left_project_column_set.push(left_col.index);
